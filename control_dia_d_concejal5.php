...
const CONCEJAL_DIA_D_PERMITIDO = 5;

function usuarioTieneAccesoConcejalDiaD(mysqli $conn): bool
{
    if (!isset($_SESSION['usuario_id'])) {
        return false;
    }

    $stmt = $conn->prepare("SELECT id_concejal FROM usuarios WHERE id = ? LIMIT 1");
    $usuarioId = (int) $_SESSION['usuario_id'];
    $stmt->bind_param('i', $usuarioId);
    $stmt->execute();
    $fila = $stmt->get_result()->fetch_assoc();

    return isset($fila['id_concejal']) && (int) $fila['id_concejal'] === CONCEJAL_DIA_D_PERMITIDO;
}
...
if (!usuarioTieneAccesoConcejalDiaD($conn)) {
    ?>
    <!DOCTYPE html>
    <html lang="es">
    <head>
        ...
        <title>Acceso denegado - Control Dia D</title>
        ...
        <main class="container py-4">
            <section class="card access-card border-0 mx-auto">
                <div class="card-body p-4 p-md-5">
                    <h1 class="h3 text-danger mb-3">Acceso no autorizado</h1>
                    <p class="text-muted mb-4">Este control esta disponible solamente para usuarios asociados al concejal ID 5.</p>
                    <div class="d-flex gap-2 flex-wrap">
                        <a href="index" class="btn btn-primary">Volver al menu</a>
...
$resC = $conn->query("SELECT id, nombre FROM concejales WHERE id = " . CONCEJAL_DIA_D_PERMITIDO . " ORDER BY nombre");
while ($resC && $fila = $resC->fetch_assoc()) {
    $concejales[] = $fila;
}

$fil_concejal = (string) CONCEJAL_DIA_D_PERMITIDO;
...
function enlaceConEstado(array $query, string $estado): string
{
    $query['estado'] = $estado;
    $query['pagina'] = 1;
    unset($query['f_concejal']);
    return 'control_dia_d_concejal5?' . http_build_query($query);
}
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Control Dia D - Concejal 5</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>
        body {
            min-height: 100vh;
    ...
    <section class="main-shell">
        <div class="hero-banner mb-4">
            <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap">
                <div>
                    <h1 class="h2 mb-2">Control Dia D - Concejal 5</h1>
                    <p class="mb-1">Cruce en tiempo real entre los votantes registrados del concejal ID 5 y el padron de quienes ya marcaron su voto.</p>
                ...
                <div class="d-flex gap-2 flex-wrap">
                    ...
                    <a href="index" class="btn btn-outline-light">Volver al menu</a>
                </div>
            </div>
        </div>
            ...
            <form method="GET" id="formFiltros">
                <div class="row g-3">
                    ...
                    <div class="col-lg-2">
                        ...
                        <input type="text" class="form-control" value="<?= htmlspecialchars($concejales[0]['nombre'] ?? 'Concejal ID 5') ?>" readonly>
                    </div>
                    <div class="col-lg-2">
                ...
                <div class="d-flex gap-2 flex-wrap mt-3">
                    ...
                    <a href="control_dia_d_concejal5" class="btn btn-outline-secondary">Limpiar</a>
                </div>
            </form>
        </section>
